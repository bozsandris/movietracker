...
public class MainActivity extends ListActivity  {
    ...
    public String url =
    "https://api.themoviedb.org/3/movie/upcoming?api_key=" + apikey + "&language=en-EN&region=GB";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if( b != null){
            ...
            DatabaseReader.deletetitle(titletodelete,mDbHelper);
        }
        setContentView(R.layout.activity_main);
        final Button refresh = findViewById(R.id.button1);
        final Button back = findViewById(R.id.button);
        final Button load = findViewById(R.id.button3);
            ...
            public void onClick(View view) {
                ...
                if(!(spinner.getSelectedItem().toString().equals( "None"))){
                    for(int i=0;i<films.size();i++){
                        if(!(films.get(i).genreids.contains(genres[spinner.getSelectedItemPosition()-1].toString()))) {films.remove(i); i--;}}
                }
        ...
        {@Override
        public void onClick(View view){
            SavedFilms.films.clear();
            Intent intent = new Intent(MainActivity.this,SavedFilms.class);
            Log.i("Intent","Starting new intent (Saved films)");
            startActivity(intent);
        }});
    }
    /***
